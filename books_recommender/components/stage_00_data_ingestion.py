...
class DataIngestion:
    ...
    def download_data(self) -> str:
        try:
            ...
            zip_downloaad_dir = self.data_ingestion_config.raw_data_dir
            os.makedirs(zip_downloaad_dir, exist_ok=True)
            data_file_name = os.path.basename(dataset_url)
            zip_file_path = os.path.join(zip_downloaad_dir, data_file_name)
            logging.info(f"Data downloading from {dataset_url} successfully and saved to {zip_file_path}")
            urllib.request.urlretrieve(dataset_url,zip_file_path)
            logging.info(f"Data downloaded from {dataset_url} successfully and saved to {zip_file_path}")
    ...
    def extract_zip_file(self, zip_file_path: str) -> None:
        try:
            ...
            logging.info(f"Extracted zip file: {zip_file_path} to dir: {ingested_dir}")
        except Exception as e:
            raise AppException(e, sys) from e    
